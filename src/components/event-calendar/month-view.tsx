...
export function MonthView({
	...
	return (
		<div data-slot="month-view" className="flex flex-col h-full">
			<div className="grid grid-cols-7 border-b border-border/70 flex-shrink-0">
				{weekdays.map((day) => (
					<div
						key={day}
			...
			<div className="grid flex-1 auto-rows-fr overflow-auto">
				{weeks.map((week, weekIndex) => (
					<div
						key={week[0]?.toISOString() || `week-${weekIndex}`}
						className="grid grid-cols-7 h-full [&:last-child>*]:border-b-0"
					>
						{week.map((day, dayIndex) => {
							...
							return (
								<div
									key={day.toString()}
									className="group flex flex-col h-full border-r border-b border-border/70 last:border-r-0 data-outside-cell:bg-muted/25 data-outside-cell:text-muted-foreground/70"
								...
								>
									...
									>
										<div className="mt-1 inline-flex size-6 items-center justify-center rounded-full text-sm flex-shrink-0 group-data-today:bg-primary group-data-today:text-primary-foreground">
											{format(day, "d")}
										</div>
										<div
